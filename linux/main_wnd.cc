...
#include "webrtc/examples/voip/linux/main_wnd.h"
...
GtkMainWnd::VideoRenderer::VideoRenderer(
    ...
    : width_(0),
     ...
     image_.reset(new uint8_t[1024 * 1024 * 4]);
...
void GtkMainWnd::VideoRenderer::SetSize(int width, int height) {
  ...
  LOG(INFO) << "frame width:" << width << " height:" << height;
  width_ = width;
  height_ = height;
...
void GtkMainWnd::VideoRenderer::OnFrame(
  ...
  gdk_threads_add_idle(Redraw, main_wnd_);*/
